                  ...
                  <div>
                     <form action="updatestock.php" method="post" enctype="multipart/form-data">
                      <div class="row">
                        ...
                        <div class="col-md-12">
						<button name="submit" type="submit" class="button button-a button-big button-rouded">Update Stock</button>
...
<?php
	...
	if (isset($_POST['submit']))
	{
		...
		if ($num!="" && $housenumber!="" && $stocknumber!="")
		{
			$query = "UPDATE stocks SET number='$num' WHERE housenumber='$housenumber' AND stocknumber='$stocknumber'";
			$query_exec=mysqli_query($conn,$query);
		}
		
		if ($num!="" && $stocknumber!="" && $housenumber!="")
		{
			$query = "UPDATE stocks SET housenumber='$housenumber' WHERE number='$num' AND stocknumber='$stocknumber'";
			$query_exec=mysqli_query($conn,$query);
		}
		
		if ($num!="" && $housenumber!="" && $name!="")
		{
			$query = "UPDATE stocks SET name='$name' WHERE number='$num' AND housenumber='$housenumber'";
			$query_exec=mysqli_query($conn,$query);
		}
		
		if ($num!="" && $housenumber!="" && $contact!="")
		{
			$query = "UPDATE stocks SET contact='$contact' WHERE number='$num' AND housenumber='$housenumber'";
			$query_exec=mysqli_query($conn,$query);
		}
		
		if ($num!="" && $housenumber!="" && $address!="")
		{
			$query = "UPDATE stocks SET address='$address' WHERE number='$num' AND housenumber='$housenumber'";
			$query_exec=mysqli_query($conn,$query);
		}
		
		if ($num!="" && $housenumber!="" && $totalpayment!="")
		{
			$query = "UPDATE stocks SET totalpayment='$totalpayment' WHERE number='$num' AND housenumber='$housenumber'";
			$query_exec=mysqli_query($conn,$query);
		}
		
		if ($num!="" && $housenumber!="" && $paidpayment!="")
		{
			$query = "UPDATE stocks SET paidpayment='$paidpayment' WHERE number='$num' AND housenumber='$housenumber'";
			$query_exec=mysqli_query($conn,$query);
		}
		...
		if ($num!="" && $housenumber!="" && $img!="")
		{
			...
			$query = "UPDATE stocks SET image='$img' WHERE number='$num' AND housenumber='$housenumber'";
			$query_exec=mysqli_query($conn,$query);
		}
